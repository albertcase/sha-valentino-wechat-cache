...
//event function end
  ...
  private function textField($key){
    $list = $this->getfilecontainer(dirname(__FILE__).'/data/keywords.php');
    if(isset($list[$key])){
  ...
  private function getFeedbacks($key){
    $list = $this->getfilecontainer(dirname(__FILE__).'/data/feedbacks.php');
    $result = array();
    if(isset($list[$key]))
      $result[0] = $list[$key];
    return $result;
  ...
  private function qrcodeSubscribeTimes($key){
    $list = $this->getfilecontainer(dirname(__FILE__).'/data/qrcodes.php');
    if(isset($list[$key]))
  ...
  private function subscribeField(){
    $list = $this->getfilecontainer(dirname(__FILE__).'/data/events.php');
  ...
  private function searchStore($latsmall, $latbig, $lngsmall, $lngbig){
    $list = $this->getfilecontainer(dirname(__FILE__).'/data/stores.php');
    $result = array();
    if($list){
